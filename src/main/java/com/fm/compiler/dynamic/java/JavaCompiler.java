...
public class JavaCompiler implements DynamicCodeCompiler {
    ...
    public Class compile(File file) throws Exception {
        if (file.getName().endsWith(JavaFileObject.Kind.SOURCE.extension)) {
            JavaFileObject javaFileObject = new JavaObject(file, JavaFileObject.Kind.SOURCE);
    ...
    private Class compile(String name, JavaFileObject javaFileObject) throws CompilerException, ClassNotFoundException {
